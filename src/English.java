...
public class English
{
  ...
  public String Execute( String Code, String OriginalCode )
  {
    ...
    if ( Code.startsWith( "-C " ) ) return Code.replace( "-C ", "" );
    
    Code = RemoveIfStart( Code, "what is the ", "" );
    Code = RemoveIfStart( Code, "what is ", "" );
    
    Code = RemoveIfStart( Code, "ask the user to ", "ask-user " );
    Code = RemoveIfStart( Code, "ask the user for ", "ask-user Enter " );
    ...
    Code = RemoveIfStart( Code, "make another ", "make a " );
    //Code = RemoveIfStart( Code, "make variable ", "make a variable "); //might cause an error
    ...
    Code = RemoveIfStart( Code, "run script ", "run-scrt " );
    
    boolean IsQuestion = Code.endsWith( "?" );
    
    /*Code = ReplaceIfEnds( Code, ".", "" ); might be a bad idea
    ...
    switch( ParsedCode[ 0 ].toLowerCase() )
    {
      ...
      case "println":
        if ( Code.indexOf( ":" ) == -1 ) // might cause the code to break
        {
          ...
          {
            ret += ParseSentence( ( String ) ToOutput.get( ToO ) ) + ( ToO == ToOutput.toArray().length - 1 ? "" : " " );
          }
      ...
      case "run-scrt":
        try
        {
          Scanner FileScaner = new Scanner( new FileInputStream( Execute( Slice( ParsedCode, 1, ParsedCode.length - 1 ) ) ) );
          String code = "";
          while( FileScaner.hasNextLine() ) code += FileScaner.nextLine() + "\n";
          SetVariable( "script output", Run( code.split( "\n" ) ) );
        }
        catch( Exception e )
        {
      ...
      break;
      case "ask-user":
        Code = Code.replaceAll( " his ", " your " ) //might be a bad idea
          .replaceAll( " her ", " your " )
          .replaceAll( " its ", " your " );
        ParsedCode = Code.split( " " );
        
        String TheQuestion = Slice( ParsedCode, 1, ParsedCode.length - 1 );
        String TheQuestionButTheFirstLetterIsCapital = new String( "" + TheQuestion.charAt( 0 ) ).toUpperCase();
        
        for( int i = 1; i < TheQuestion.length(); i++ ) TheQuestionButTheFirstLetterIsCapital += TheQuestion.charAt( i );
        
        System.out.print( ( Execute( TheQuestion ).equals( TheQuestion ) ? TheQuestionButTheFirstLetterIsCapital : Execute( TheQuestion ) ) + ": " );
  ...
  {
    Sentence = RemoveIfStart( Sentence, "the ", "" );
